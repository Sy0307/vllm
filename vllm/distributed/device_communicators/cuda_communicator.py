...
class CudaCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        self._bounded_symm_scratch_enabled = bool(
    ...
    def all_reduce(self, input_):
        ...
        if (
            ...
            if self._bounded_symm_scratch_enabled:
                ...
                if out is not None:
                    # The registered output is a single reusable scratch slot.
                    # Some callers retain consecutive all-reduce results (for
                    # example draft embeddings on the last PP stage), so never
                    # let that storage escape. The copy is ordered after NCCL
                    # on the current stream and ordinary allocator storage can
                    # safely live across the next scratch reuse.
                    out = out.clone()
            else:
    ...
    def _get_symm_scratch(
        ...
        the upstream mode.  The bounded mode instead maintains a power-of-two
        ...
        if bounded is None:
            bounded = bool(int(os.getenv("VLLM_NCCL_SYMM_MEM_BOUNDED_SCRATCH", "0")))
            self._bounded_symm_scratch_enabled = bounded
        if bounded:
            # A long-prefill batch can visit many nearby token counts. NCCL
            # keeps every symmetric-memory segment registered, so caching by
            # exact shape grows without bound. Reuse a flat power-of-two
            # capacity per role/dtype/device. Keep every retired generation
            # alive: CUDA graphs can retain its address, and NCCL's private
            # pool does not release/deregister it while the pool is alive.
            # Power-of-two growth bounds all retired generations to less than
            # the final capacity, so total live+retired storage is <2x final.
            ...
            if bool(int(os.getenv("VLLM_NCCL_SYMM_MEM_ALIAS_DIAGNOSTICS", "0"))):
    ...
    def seal_bounded_symm_scratch(self, reason: str) -> None:
        """Mark startup profiling complete and diagnose any later growth."""
