...
def test_no_spec_prefill_to_recoverssm_target_state_transport(
    ...
    """Only common target state crosses P/D; every D replica initializes extras."""
    num_blocks = 4
    prefill = KDATargetStateLayerTransport(
        "linear_attn",
        1,
        _page_cache(_prefill_spec(), num_blocks),
        _prefill_spec(),
    ...
    p_regions = prefill.regions
    assert [region.kind for region in p_regions] == [
        KDA_TARGET_CONV_REGION,
        KDA_BASE_RECURRENT_REGION,
    ]
    ...
    for _ in range(dcp_size):
        decode = KDATargetStateLayerTransport(
            "linear_attn",
            1,
            _page_cache(_recover_spec(num_speculative_tokens), num_blocks),
            _recover_spec(num_speculative_tokens),
            conv_state_dim_first=True,
        )
        for state in decode.states:
            state.fill_(99)

        d_regions = decode.regions
        assert [region.content_len_bytes for region in d_regions] == [
            region.content_len_bytes for region in p_regions
        ]
        ...
        for record in decode.states[2:]:
            assert torch.all(record[0] == 99)


@pytest.mark.cpu_test
def test_homogeneous_recoverssm_transport_still_resets_local_records():
...
@pytest.mark.cpu_test
def test_checksum_splits_target_and_draft_attention(monkeypatch, caplog):
    monkeypatch.setenv("VLLM_KDA_TRANSPORT_CHECKSUM", "1")
    target_page = torch.zeros((2, 8), dtype=torch.uint8)
    draft_page = torch.ones((2, 8), dtype=torch.uint8)
    transport = KDATargetStateTransport(
        {},
        {
            "model.layers.1.self_attn": (0, target_page, 1),
            "model.layers.2.self_attn": (1, draft_page, 1),
        },
        target_num_layers=2,
    )

    with caplog.at_level(logging.INFO):
        transport.debug_checksums("D_DIRECT", "req", {0: [0], 1: [0]})
